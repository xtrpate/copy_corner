...
# --- Get Arguments Passed from Register.py ---
try:
    user_fullname = sys.argv[1]
    user_username = sys.argv[2]
    user_email = sys.argv[3]
    user_password = sys.argv[4]
    user_contact = sys.argv[5]
    correct_otp = sys.argv[6]
except IndexError:
    messagebox.showerror("Error", "Could not load user data. Please try registering again.")
    user_fullname, user_username, user_email, user_password, user_contact, correct_otp = [""] * 6
...
def create_user_account():
    ...
    try:
        cursor = conn.cursor()
        insert_query = """
        INSERT INTO users (fullname, username, email, password, contact)
        VALUES (%s, %s, %s, %s, %s)
        """
        password_bytes = user_password.encode('utf-8')
        hashed_password = bcrypt.hashpw(password_bytes, bcrypt.gensalt())
        user_data = (user_fullname, user_username, user_email, hashed_password, user_contact)
        cursor.execute(insert_query, user_data)
        conn.commit()
        return True
    except mysql.connector.Error as err:
        messagebox.showerror("Database Error", f"Could not create account: {err}")
        return False
    finally:
...
def verify_otp():
    ...
    if entered_otp == correct_otp:
        if create_user_account():
            messagebox.showinfo("Success", "Email verified and account created! You can now log in.")
            window.destroy()
            subprocess.Popen([sys.executable, "login_frame.py"]) # Or main.py?
        # else: DB error already shown
    else:
...
def resend_otp():
    ...
    if send_verification_email(user_email, new_otp): # Default subject/context
        messagebox.showinfo("Code Resent", f"A new 6-digit code has been sent to {user_email}.")
        window.destroy()
        subprocess.Popen([
            sys.executable, "OTP.py",
            user_fullname, user_username, user_email, user_password, user_contact, new_otp
        ])
...
window.configure(bg="#FFFFFF"); window.title("Email Verification")

canvas = Canvas(window, bg="#FFFFFF", height=500, width=400, bd=0, highlightthickness=0, relief="ridge")
canvas.place(x=0, y=0)

# --- UI Elements ---
canvas.create_text(window_width / 2, 60, anchor="center", text="Email Verification", fill="#000000", font=("Inter Bold", 24))
...
verify_btn_text = canvas.create_text(window_width / 2, 345, anchor="center", text="Verify Account", fill="#FFFFFF", font=("Inter Bold", 14))
